...
//halving
    ...
    // value of now.
  // In the stake function, update the farmRate mapping for both the staker and the devWallet
function stake(uint256 _tokenId) external nonReentrant {
    // If wallet has tokens staked, calculate the rewards before adding the new token
    if (stakers[msg.sender].amountStaked > 0) {
        uint256 rewards = calculateRewards(msg.sender);
        stakers[msg.sender].unclaimedRewards += rewards;
        uint256 devrewards = calculateRewards(devWallet);
        stakers[devWallet].unclaimedRewards += devrewards;
    }
    ...
    // nftCollection.transferFrom(msg.sender, address(this), _tokenId);//lock transfer instead
    ...
    farmRate[devWallet]+=nft.farmRate(_tokenId)*devRate;
    ...
    stakers[devWallet].timeOfLastUpdate = block.timestamp;
    ...
    function modifyRates(uint _devRate, uint _userRate)public onlyRole(AUTHORIZED_ROLE) {
        devRate=_devRate;
        userRate=_userRate;
    }
